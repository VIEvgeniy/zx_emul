...
void convert_kb_u_to_kb_zx(kb_u_state* kb_st,uint8_t* zx_kb)
{
    ...
    if (kb_st->u[2])
    {
            ...
            if (kb_st->u[2]&KB_U2_UP) {zx_kb[0]|=1<<0;zx_kb[4]|=1<<3;};
            if (kb_st->u[2]&KB_U2_DOWN) {zx_kb[0]|=1<<0;zx_kb[4]|=1<<4;};
            if (kb_st->u[2]&KB_U2_LEFT) {zx_kb[0]|=1<<0;zx_kb[3]|=1<<4;};
            if (kb_st->u[2]&KB_U2_RIGHT) {zx_kb[0]|=1<<0;zx_kb[4]|=1<<2;};
    }
