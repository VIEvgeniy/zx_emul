...
//------------------------
 inline  bool FAST_FUNC(get_random)(){
...
{   
    ...
    if (nR7&0x1) {chA_count+=delta;if (chA_count>=ay_R1_R0 && (ay_R1_R0>=delta) ) {chA_bit^=1;chA_count=0;}} else {chA_bitOut=1;chA_count=0;};
    if (nR7&0x2) {chB_count+=delta;if (chB_count>=ay_R3_R2 && (ay_R3_R2>=delta) ) {chB_bit^=1;chB_count=0;}} else {chB_bitOut=1;chB_count=0;};
    if (nR7&0x4) {chC_count+=delta;if (chC_count>=ay_R5_R4 && (ay_R5_R4>=delta) ) {chC_bit^=1;chC_count=0;}} else {chC_bitOut=1;chC_count=0;};
    
    // chA_count+=delta;
    // chB_count+=delta;
    // chC_count+=delta;
    // if (nR7&0x1) {if (chA_count>=ay_R1_R0 && (ay_R1_R0>=delta) ) {chA_bit^=1;chA_count-=ay_R1_R0;}} else {chA_bitOut=1;};
    // if (nR7&0x2) {if (chB_count>=ay_R3_R2 && (ay_R3_R2>=delta) ) {chB_bit^=1;chB_count-=ay_R3_R2;}} else {chB_bitOut=1;};
    // if (nR7&0x4) {if (chC_count>=ay_R5_R4 && (ay_R5_R4>=delta) ) {chC_bit^=1;chC_count-=ay_R5_R4;}} else {chC_bitOut=1;};
   ...
   // uint32_t ay_R12_R11_sh=ay_R12_R11<<1;
    ...
    // outA=chA_bitOut?(ampls0[ay_R8]):0;
    // outB=chB_bitOut?(ampls0[ay_R9]):0;
    // outC=chC_bitOut?(ampls0[ay_R10]):0;
